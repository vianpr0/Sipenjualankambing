            ...
            <div class="md:w-full w-full bg-white p-2 shadow mr-8">
                                ...
                                @foreach ($posts as $post)
                                    <div class="flex flex-col items-center gap-4 md:flex-row lg:gap-6">
                                        <a href="#"
                                            ...
                                            <img src="{{ $post->Image }}"
                                                loading="lazy" alt="{{ $post->Nama_Article }}"
                                                class="absolute inset-0 h-full w-full object-cover object-center transition duration-200 group-hover:scale-110" />
                                        </a>
                                        <div class="flex flex-col gap-2">
                                            <span class="text-sm text-gray-400">{{ \Carbon\Carbon::parse($post->published_at)->translatedFormat('j F Y') }}</span>
                                            ...
                                            <div>
                                                <a href="/index/berita/{{ $post['Url'] }}"
                                                    class="font-semibold text-indigo-500 transition duration-100 hover:text-indigo-600 active:text-indigo-700">Read more</a>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="mt-6 flex justify-around">
                    <div class="join grid grid-cols-2 gap-4">
                        ...
                        @else
                            <a href="{{ $posts->previousPageUrl() }}" class="join-item btn btn-outline btn-warning">Previous page</a>
                        @endif
